...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await actor.submitLead(
        ...
        formData.phone,
        null,                   // email (optional)
        formData.zip || null,   // zip (optional)
      ...
      setError(`Something went wrong. Please call us directly at ${BUSINESS_PHONE}.`);
    } finally {
    ...
    return (
      <div className="bg-white rounded-2xl p-8 shadow-navy-md text-center">
        ...
        <p className="text-gray-600 mb-4">
          Thanks, {formData.name}! Dom will call you back within 1 hour during business hours.
        </p>
        <a
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 shadow-navy-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          <label className="block text-sm font-semibold text-brand-navy-800 mb-1.5">
            Your Name *
          </label>
          <input
            type="text"
