...
const SERVICES = [
  { name: 'Emergency Plumbing', icon: AlertTriangle, color: 'text-red-500', bg: 'bg-red-50 border-red-200' },
  { name: 'Drain Cleaning', icon: Droplets, color: 'text-brand-navy-600', bg: 'bg-brand-navy-50 border-brand-navy-200' },
  { name: 'Leak Repair', icon: Wrench, color: 'text-brand-orange-500', bg: 'bg-brand-orange-50 border-brand-orange-200' },
  { name: 'Plumbing & Sewer', icon: Zap, color: 'text-brand-navy-600', bg: 'bg-brand-navy-50 border-brand-navy-200' },
  { name: 'Handyman Services', icon: Wrench, color: 'text-brand-orange-500', bg: 'bg-brand-orange-50 border-brand-orange-200' },
  { name: 'Painting', icon: Paintbrush, color: 'text-brand-navy-600', bg: 'bg-brand-navy-50 border-brand-navy-200' },
  { name: 'Power Washing', icon: Wind, color: 'text-brand-orange-500', bg: 'bg-brand-orange-50 border-brand-orange-200' },
  { name: 'Cleaning Services', icon: Sparkles, color: 'text-brand-navy-600', bg: 'bg-brand-navy-50 border-brand-navy-200' },
];
...
export default function BookingModal() {
  ...
  const totalSteps = 5; // 1:service, 2:type, 4:details, 5:contact, 6:confirm (3 is emergency dead-end)
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3 mt-2">
            {SERVICES.map(({ name, icon: Icon, color, bg }) => (
              ...
              >
                ...
                <span className="text-brand-navy-900 font-semibold text-sm leading-tight">{name}</span>
              </button>
            ))}
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4 mt-2">
            <p className="text-brand-navy-700 text-sm">
              You selected: <span className="font-bold text-brand-orange-600">{form.service}</span>
            </p>
            <p className="text-brand-navy-600 text-sm">What type of request is this?</p>
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="text-center">
                  <div className="font-bold text-brand-navy-900 text-sm">Get an Estimate</div>
                  <div className="text-brand-navy-500 text-xs mt-1">Schedule a visit & get pricing</div>
                </div>
              </button>
              <button
              ...
              >
                ...
                <div className="text-center">
                  <div className="font-bold text-red-700 text-sm">Emergency</div>
                  <div className="text-red-500 text-xs mt-1">Needs immediate attention</div>
                </div>
              </button>
            </div>
            <Button
          ...
          <div className="text-center space-y-5 py-4">
            ...
            <div>
              ...
              <p className="text-brand-navy-600 text-sm leading-relaxed">
                For immediate assistance, please call us right now. We're available 24/7 for emergencies.
              </p>
            </div>
            <a
        ...
        {step === 4 && (
          <div className="space-y-4 mt-2">
            <p className="text-brand-navy-700 text-sm">
              Service: <span className="font-bold text-brand-orange-600">{form.service}</span>
            </p>
            <div className="space-y-2">
              <Label htmlFor="details" className="text-brand-navy-800 font-semibold">
              ...
              />
              <p className="text-brand-navy-400 text-xs">Optional but helps us prepare for your visit.</p>
            </div>
        ...
        {step === 5 && (
          <div className="space-y-4 mt-2">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-brand-navy-800 font-semibold">
            ...
            <div className="space-y-2">
              <Label htmlFor="phone" className="text-brand-navy-800 font-semibold">
            ...
            <div className="space-y-2">
              <Label htmlFor="email" className="text-brand-navy-800 font-semibold">
            ...
            {submitLead.isError && (
              <p className="text-red-600 text-sm bg-red-50 border border-red-200 rounded-lg px-3 py-2">
                Something went wrong. Please try again or call us directly at {BUSINESS_PHONE}.
              </p>
            )}
            <div className="flex gap-3 pt-2">
              <Button
        ...
        {step === 6 && (
          <div className="text-center space-y-5 py-4">
            ...
            <div>
              ...
              <p className="text-brand-navy-600 text-sm leading-relaxed">
                Thanks, <span className="font-semibold">{form.name}</span>! We'll reach out to{' '}
                <span className="font-semibold">{form.phone}</span> shortly to confirm your{' '}
                <span className="font-semibold text-brand-orange-600">{form.service}</span> appointment.
              </p>
            </div>
