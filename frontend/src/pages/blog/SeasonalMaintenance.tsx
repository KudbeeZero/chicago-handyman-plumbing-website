...
import { Phone, Calendar, CheckCircle, Sun, Snowflake, Leaf, Cloud } from "lucide-react";
      ...
      <article className="py-16 bg-white">
        <div className="max-w-3xl mx-auto px-4">
          <p className="text-gray-700 mb-8 text-lg leading-relaxed">
            Chicago homeowners face unique challenges. Temperatures swing from -20°F in winter to 95°F in summer. Spring brings heavy rains that test sewer systems. Fall means leaves clogging gutters. Staying ahead of seasonal maintenance is the best way to protect your home and avoid expensive emergency repairs.
          </p>

          {/* Season checklists */}
          <div className="flex flex-col gap-8 mb-10">
            {seasons.map(({ name, icon: Icon, color, bg, border, tasks }) => (
              <div key={name} className={`${bg} rounded-2xl p-6 border ${border}`}>
                <div className="flex items-center gap-3 mb-4">
                  <div className={`${bg} rounded-xl p-2 border ${border}`}>
                    <Icon className={`h-6 w-6 ${color}`} />
                  </div>
                  <h2 className="text-2xl font-heading font-black text-brand-navy-900">{name} Checklist</h2>
                </div>
                <div className="grid sm:grid-cols-2 gap-2">
                  {tasks.map((task) => (
                    <div key={task} className="flex items-start gap-2">
                      <CheckCircle className={`h-4 w-4 ${color} shrink-0 mt-0.5`} />
          ...
          <div className="grid sm:grid-cols-3 gap-4 mb-10">
            ...
            <Link to="/services/power-washing" className="bg-gray-50 rounded-2xl p-5 border border-gray-100 hover:border-brand-orange-300 transition-all">
              <h3 className="font-heading font-bold text-brand-navy-900 mb-1">Power Washing</h3>
          ...
          <div className="bg-brand-navy-900 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h3 className="font-heading font-black text-white text-xl">Need Help With Seasonal Maintenance?</h3>
              <p className="text-brand-navy-300 text-sm mt-1">Call Dom for a free estimate on any home maintenance project.</p>
            </div>
